...
# This will compile go-ipfs v0.12.0 and the current sources of our plugin to match that.
# Sources are placed in /usr/src/ipfs/.
# Artifacts are put in /usr/local/bin/ipfs/.

# We need go v1.16 because of a weird compiler bug in 1.17.
...
# First, get and compile go-ipfs v0.12.0.
...
RUN git clone https://github.com/ipfs/go-ipfs.git . && git checkout v0.12.0
RUN go build -v -o /usr/local/bin/ipfs/ipfs-v0.12.0-docker ./cmd/ipfs
...
RUN go build -v -buildmode=plugin -o /usr/local/bin/ipfs/mexport-v0.12.0-docker.so main.go

# Go plugins need to be executable.
RUN chmod +x /usr/local/bin/ipfs/mexport-v0.12.0-docker.so

# Artifacts should now be in /usr/local/bin/ipfs-stuff
