...
# This will compile kubo v0.18.0 and the current sources of our plugin to match that.
...
# First, get and compile kubo v0.18.0.
...
RUN git clone https://github.com/ipfs/kubo.git . && git checkout v0.18.0
RUN go build -v -o /usr/local/bin/ipfs/ipfs-v0.18.0-docker ./cmd/ipfs
...
RUN go build -v -buildmode=plugin -o /usr/local/bin/ipfs/mexport-v0.18.0-docker.so main.go

# Go plugins need to be executable.
RUN chmod +x /usr/local/bin/ipfs/mexport-v0.18.0-docker.so
...
FROM ipfs/kubo:v0.18.0

# Copy artifacts from builder.
COPY --from=builder /usr/local/bin/ipfs/ipfs-v0.18.0-docker /usr/local/bin/ipfs
COPY --from=builder /usr/local/bin/ipfs/mexport-v0.18.0-docker.so /mexport-plugin/