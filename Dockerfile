...
# This will compile kubo v0.21.0-rc2 and the current sources of our plugin to match that.
...
# First, get and compile kubo v0.21.0-rc2.
...
RUN git clone https://github.com/ipfs/kubo.git . && git checkout v0.21.0-rc2
RUN go build -v -o /usr/local/bin/ipfs/ipfs-v0.21.0-rc2-docker ./cmd/ipfs
...
RUN go build -v -buildmode=plugin -o /usr/local/bin/ipfs/mexport-v0.21.0-rc2-docker.so main.go

# Go plugins need to be executable.
RUN chmod +x /usr/local/bin/ipfs/mexport-v0.21.0-rc2-docker.so
...
FROM ipfs/kubo:v0.21.0-rc2

# Copy artifacts from builder.
COPY --from=builder /usr/local/bin/ipfs/ipfs-v0.21.0-rc2-docker /usr/local/bin/ipfs
COPY --from=builder /usr/local/bin/ipfs/mexport-v0.21.0-rc2-docker.so /mexport-plugin/
